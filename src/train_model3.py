"""
Model 3: AdaBoost Training Script

This script trains an AdaBoost model using scikit-learn and logs everything to MLflow.
Alternative to XGBoost (third-best performer from AutoML).
"""
...
def create_plots(model, feature_cols, y_test, y_test_pred):
    ...
    ax.set_title('Top 15 Feature Importances - AdaBoost Model')
    ax.set_xlabel('Importance')
    plt.tight_layout()
    feature_importance_path = plots_dir / "model3_adaboost_feature_importance.png"
    ...
    ax.set_title('Residual Plot - AdaBoost Model')
    plt.tight_layout()
    residual_path = plots_dir / "model3_adaboost_residuals.png"
    ...
    ax.set_title('Actual vs Predicted - AdaBoost Model')
    plt.tight_layout()
    pred_path = plots_dir / "model3_adaboost_predictions.png"
...
def log_to_mlflow(model, params, metrics, feature_importance_path, residual_path, pred_path):
    ...
    with mlflow.start_run(run_name="AdaBoost_Model"):
        # Log parameters
        ...
        mlflow.sklearn.log_model(
            model, 
            "model",
            registered_model_name="PM25_AdaBoost"
        )
        ...
        mlflow.set_tag("algorithm", "sklearn.AdaBoostRegressor")
...
def main():
    ...
    print("\n" + "="*70)
    print("ADABOOST MODEL TRAINING COMPLETED SUCCESSFULLY!")
    print("="*70)
