"""
Model 1: Gradient Boosting Machine (GBM) Training Script

This script trains a GBM model using LightGBM and logs everything to MLflow.
Based on H2O AutoML top performer.
"""
...
def train_model(X_train, y_train, X_val, y_val):
    ...
    print("\nTraining Gradient Boosting (GBM) model...")
    
    # Model parameters (similar to H2O AutoML GBM defaults)
    params = {
        'n_estimators': 100,
        'learning_rate': 0.1,
        'max_depth': 6,
...
def create_plots(model, feature_cols, y_test, y_test_pred):
    ...
    ax.set_title('Top 15 Feature Importances - GBM Model')
    ax.set_xlabel('Importance')
    plt.tight_layout()
    feature_importance_path = plots_dir / "model1_gbm_feature_importance.png"
    ...
    ax.set_title('Residual Plot - GBM Model')
    plt.tight_layout()
    residual_path = plots_dir / "model1_gbm_residuals.png"
    ...
    ax.set_title('Actual vs Predicted - GBM Model')
    plt.tight_layout()
    pred_path = plots_dir / "model1_gbm_predictions.png"
...
def log_to_mlflow(model, params, metrics, feature_importance_path, residual_path, pred_path):
    ...
    with mlflow.start_run(run_name="GBM_Model"):
        # Log parameters
        ...
        mlflow.sklearn.log_model(
            model, 
            "model",
            registered_model_name="PM25_GBM"
        )
        ...
        mlflow.set_tag("algorithm", "sklearn.GradientBoostingRegressor")
...
def main():
    ...
    print("="*70)
    print("MODEL 1: GRADIENT BOOSTING MACHINE (GBM)")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("GBM MODEL TRAINING COMPLETED SUCCESSFULLY!")
    print("="*70)
